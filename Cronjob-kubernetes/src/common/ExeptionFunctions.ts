...
import { AppError, AppErrorToSent } from "./AppError";
...
export class ExeptionFunctions {
  ...
  public static GetAppError = (error: any): AppError => {
    ...
    if ((error.name as string).startsWith("AxiosError")) {
      appError = ExeptionFunctions.Parse_AxiosError(error);
    }
  ...
  public static Parse_AxiosError = (error: AxiosError): AppError => {
    let message;
    let errorCode;
    ...
    const err: AppError = new AppError(statusCode, errorCode, message, ErrorTypeEnum.TecnicalException);
  ...
  public static Parse_SequelizeError = (error: any): AppError => {
    ...
    if (!message && error.name === "SequelizeUniqueConstraintError") {
      errorCode = ErrorCodeEnum.SEQUALIZE_DATA;
      if (error.errors) {
