...
import { HashDTO } from "./DTOs/Auth/HashDTO";
...
export default class AuthService implements IAuthService {
  ...
  public async Hash(value: string): Promise<HashDTO> {

    const secureKey = CryptoFunctions.GenerateSecureKey();
    const hashedVal = CryptoFunctions.Hash(value, secureKey);
    const hashedVal2 = await this.userRepository.HassPassword(value);
    const res: HashDTO = {
      hashedVal,
      hashedVal2,
      secureKey
