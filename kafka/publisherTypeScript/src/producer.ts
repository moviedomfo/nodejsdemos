...
export class Publisher {
  ...
  public async Start() {
    ...
    Helper.LogConsole(  `------------------Publisher started  ${publisherName || ''} --------------------`);
  ...
  private async SendProvider(person: Person): Promise<any> {
    const url = AppSettings.BASE_URL + "/api/personspubs/provider";
    ...
    return new Promise<any>((resolve, reject) => {
      return axios.post<any>(url,data,{headers: AppSettings.HEADERS})
        .then((res) => {
  
          resolve(res.data);
        })
        .catch(function (error) {
          let e = new Error("Importing finalized with errors : " +   Helper.GetError(error));
          reject(e);
        });
    });
  ...
  private async SendCustomer(person: Person): Promise<any> {
    const url = AppSettings.BASE_URL + "/api/personspubs/customer";
